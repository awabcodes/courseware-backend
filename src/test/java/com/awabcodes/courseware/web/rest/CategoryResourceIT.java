...
import com.awabcodes.courseware.service.dto.CategoryCriteria;
import com.awabcodes.courseware.service.CategoryQueryService;
...
public class CategoryResourceIT {
    ...
    @Test
    @Transactional
    public void getAllCategoriesByCoursesIsEqualToSomething() throws Exception {
        // Initialize the database
        categoryRepository.saveAndFlush(category);
        Course courses = CourseResourceIT.createEntity(em);
        em.persist(courses);
        em.flush();
        category.addCourses(courses);
        categoryRepository.saveAndFlush(category);
        Long coursesId = courses.getId();

        // Get all the categoryList where courses equals to coursesId
        defaultCategoryShouldBeFound("coursesId.equals=" + coursesId);
