...
import com.awabcodes.courseware.repository.TagRepository;
...
public class TagResourceIT {
    ...
    @Test
    @Transactional
    public void getAllTagsByCoursesIsEqualToSomething() throws Exception {
        // Initialize the database
        tagRepository.saveAndFlush(tag);
        Course courses = CourseResourceIT.createEntity(em);
        em.persist(courses);
        em.flush();
        tag.addCourses(courses);
        tagRepository.saveAndFlush(tag);
        Long coursesId = courses.getId();

        // Get all the tagList where courses equals to coursesId
        defaultTagShouldBeFound("coursesId.equals=" + coursesId);

        // Get all the tagList where courses equals to coursesId + 1
        defaultTagShouldNotBeFound("coursesId.equals=" + (coursesId + 1));
    }
